...
function normalizeAndReplace(target, replacement, label) {
...
// 1. Update Card 1 Substats (lines ~15749)
const targetCard1 = `<div className="text-[10px] text-gray-500 dark:text-gray-400 truncate flex items-center gap-1.5 font-semibold mt-0.5">
...
const replaceCard1 = `<div className="text-[10px] text-gray-500 dark:text-gray-400 truncate flex items-center gap-1.5 font-semibold mt-0.5 flex-wrap">
                                            <span className="text-cyan-600 dark:text-cyan-400">{(compComparisonStats.pickerCount || 0).toLocaleString('id-ID')} Picker</span>
                                            <span>•</span>
                                            <span className="text-amber-600 dark:text-amber-400">{(compComparisonStats.ojolCount || 0).toLocaleString('id-ID')} Ojol</span>
...
normalizeAndReplace(targetCard1, replaceCard1, '1. Card 1 Substats');

// 2. Update Picker Header & Filter Pills (lines ~15944)
const targetPickerHeader = `<div className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-2">
...
const replacePickerHeader = `<div className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-1.5 flex-wrap font-medium">
                                                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Match: {(compComparisonStats.matchCount || 0).toLocaleString('id-ID')}</span>
                                                  <span>•</span>
...
normalizeAndReplace(targetPickerHeader, replacePickerHeader, '2. Picker Header & Filter Pills');

// 3. Update Picker Table Rows (lines ~16120-16202)
const targetPickerRow = `                                                     <tr
                                                        key={item.id || idx}
...
normalizeAndReplace(targetPickerRow, replacePickerRow, '3. Picker Table Rows');

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully completed patching AdminDashboard.tsx!');
