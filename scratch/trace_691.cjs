...
async function run() {
  ...
  console.log('pickerRaw count:', pickerRaw.length);
  const targetInPickerRaw = pickerRaw.find(x => x.barcode === '006912414671');
  console.log('targetInPickerRaw:', targetInPickerRaw);
  ...
  console.log('targetInCombinedPickerRaw:', combinedPickerRaw.find(x => x.barcode === '006912414671'));
  ...
  const activePickerRaw = combinedPickerRaw.filter((item) => {
  ...
  console.log('targetInActivePickerRaw:', activePickerRaw.find(x => x.barcode === '006912414671'));

  const formattedPicker = activePickerRaw.map((item) => {
  ...
  const foundFormatted = formattedPicker.find(x => x.barcode === '006912414671');
  console.log('foundFormatted in formattedPicker:', foundFormatted);

  // Now test search filter on filteredPickerComparisonList:
  const search = '006912414671';
  const filtered = formattedPicker.filter(item => {
     if (search.trim()) {
        ...
        if (!b.includes(s) && !e.includes(s) && !l.includes(s) && !r.includes(s)) return false;
     }
     return true;
  });
  console.log('Search "006912414671" on formattedPicker results:', filtered);
}
